...
import ws.rocket.sqlstore.types.ShortMapper;

/**
 * Tests for the {@link ShortMapper} class.
 */
@Test
public final class ShortMappingTest {

  private final ShortMapper mapper = new ShortMapper();

  public void shouldAcceptInteger() {
    assertTrue(this.mapper.supports(short.class));
    assertTrue(this.mapper.supports(Short.class));
  }
  ...
  public void shouldWriteIntToStatement() throws SQLException {
    ...
    this.mapper.write(stmt, 1, (short) 2, Types.NUMERIC);

    verify(stmt).setShort(1, (short) 2);
  }
  ...
  public void shouldReadShortFromStatement() throws SQLException {
    CallableStatement stmt = mock(CallableStatement.class);
    when(stmt.getShort(1)).thenReturn((short) 2);
    when(stmt.wasNull()).thenReturn(false);

    assertEquals(this.mapper.read(stmt, 1, Types.NUMERIC), Short.valueOf((short) 2));
  }
  ...
  public void shouldReadShortFromResultSet() throws SQLException {
    ResultSet rs = mock(ResultSet.class);
    when(rs.getShort(1)).thenReturn((short) 2);
    when(rs.wasNull()).thenReturn(false);

    assertEquals(this.mapper.read(rs, 1, Types.BOOLEAN), Short.valueOf((short) 2));
  }
