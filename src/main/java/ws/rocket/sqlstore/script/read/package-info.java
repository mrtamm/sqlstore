 ...
 * <p>
 * The functionality of parsing is divided depending on the level of interpretation being handled:
 * <ol>
 ...
 * <h2>The SQLS File</h2>
 * <p>
 * <code>SqlStore</code> introduces a custom file format describing multiple SQL scripts, their
 ...
 * <p>
 * The structure of this file is simple. At the beginning, there are optional custom Java type
 ...
 * <pre>
 * !Employee=org.company.data.Employee
 * !E=org.company.data.Employee
 * </pre>
 * <p>
 * The previous example specifies that Java types "Employee" and "E" actually refer to class
 * <code>org.company.data.Employee</code>.
 * <p>
 * After optional Java type mappings there are stored SQL scripts. Each script begins with a name,
 * optional parameters and query hints, and an SQL script in curly braces. Whitespace (including
 * newlines) is allowed between these elements but all this information can also be contained in one
 * line, as well.
 * <p>
 * The overall script syntax is following and a file can contain multiple such scripts:
 ...
 * <h4>Script Name</h4>
 * <p>
 * The name is used for enabling a Java code to refer to a specific script to be executed in an
 ...
 * <p>
 * The script name can be summarized as following:
 * <pre>script-name := &lt;a valid Java identifier&gt;</pre>
 *
 * <h4>Parameters And Hints</h4>
 * <p>
 * There are three types of possible parameters: <code>IN</code>, <code>OUT</code>, and
 ...
 * Therefore, <code><em>parameters-hints</em></code> clause has following syntax:
 * <pre>
 * parameters-hints := {
 ...
 * </pre>
 * <p>
 * <strong>NOTE:</strong> no whitespace is allowed before opening parenthesis! For example:
 * <ol>
 ...
 * <p>
 * When talking about parameters, it is important to note that all parameters must have a Java type,
 * and an optional SQL type (a constant name from {@link java.sql.Types}). The Java type must be one
 * of the following:
 * <ol>
 ...
 * </ol>
 * <p>
 * The Java type is used for choosing the most approach to setting or reading a statement parameter.
 * It can also be influenced by the SQL type of the parameter.
 * <p>
 * When SQL type is present, it is separated with a pipe (<code>|</code>) character from the Java
 ...
 * <p>
 * When input parameters are required, they are represented as a comma separated list of named
 ...
 * <p>
 * The rules for the parameter name are the same as for the script name: it must be valid Java
 * identifier, and it must be unique within the context of the script where it is used.
 * <p>
 * Therefore nothing complicated as shown in these examples:
 * <pre>
 * IN(Long id, java.math.BigDecimal amount)
 ...
 * <p>
 * When the script returns a <em>result-set</em> after execution and each row must be converted to a
 * bean, the type parameter is just for the case. When the value from the result-set must be set as
 * it is, just use the type name. Otherwise, when there are several columns to be converted into
 * bean, also specify the properties for each column. Here are examples for both scenarios:
 * <pre>
 * # Returns a map with Long values as keys, and String values as values.
 ...
 * OUT(Long, Employee[id, name, birthDate])
 * </pre>
 *
 * <h4>Hints for SQL Script</h4>
 * <p>
 * SqlStore does not do some guessing to optimize a script execution. It is so to avoid bad
 ...
 * <h4>Expressions</h4>
 * <p>
 * An SQL script can make use of parameters by referring to them using expressions. These are marked
 * within symbols <code>${...}</code> in an SQL script, and are replaced with question marks, which
 * are already familiar to JDBC API users. An expression typically begins with a reference to a
 * parameter name and can refer to any number of (nested) properties using dot-notation as
 ...
 * <p>
 * In addition, expressions also are aware of Java and SQL type of the expression value. The Java
 ...
 * Here are examples:
 * <pre>
 * # Injects input parameters "id", "name", and "birthDate":
 * INSERT INTO employee(id, name, birth_date)
 * VALUES (${id}, ${name}, ${birthDate|DATE});
 *
 * # Injects the properties of an input parameter "e":
 * INSERT INTO employee(id, name, birth_date)
 * VALUES (${e.id}, ${e.name}, ${e.birthDate|DATE});
 * </pre>
 *
 * <h4>OUT-Expressions</h4>
 * <p>
 * In more advanced scenarios, mostly related to calling database procedures, it may be necessary to
 * read back the values of the script (after being modified by a procedure call). An expression in
 * the script may be wrapped into <code>OUT(...)</code> or <code>INOUT(...)</code>. The former
 ...
 * <p>
 * JDBC supports two kinds of result-sets: one for reading query results, other for reading
 ...
 * <p>
 * These parameters are more like expressions to be evaluated after query, especially when the
 * expression is not directly used in a query. Here is an example to illustrate a use case:
 * <pre>
 * UPDATE(KEYS(e.id))
 * # for SQL script
 * INSERT INTO employee(id, name, birth_date)
 * VALUES (emp_seq.nextval, ${e.name}, ${e.birthDate|DATE});
 * </pre>
 * <p>
 * In the previous example, the use of <code>KEYS(...)</code> signals that the auto-generated keys
 * are expected after query. The auto-generated key is from sequence named <code>emp_seq</code>. The
 * key will be stored in the property <code>id</code> of the input parameter named <code>e</code>.
 * This script most likely does not have any output parameters, since the generated ID will be
 * stored in the input parameter right after the execution of the script.
 *
 * <h4>Script Validation</h4>
 * <p>
 * During the parsing process most of the script information will be validated. Only the SQL script
 * itself cannot be validated for correctness as it can be verified only by database. An error will
 * abort the loading process by throwing {@link ws.rocket.sqlstore.ScriptSetupException}.
 * <p>
 * Examples of items that are checked during script parsing:
 * <ol>
