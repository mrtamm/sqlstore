 ...
 *
 * <p>The runtime model of a {@literal Script} must be capable of
 * <ol>
 ...
 * </ol>
 *
 * <p>For better performance, information about a {@literal Script} is not maintained in the same
 ...
 *
 * <p>All in all, once parsing is completed, the best effort has been made to detect problems with
 * query, and assuming that query gets correct parameters, the query should return results in
 * correct format. A {@literal Script} object can be sure that the query string is non-empty but
 * cannot guarantee its correctness. That can be verified only by executing the statement.
 *
 * <p>Currently, there is only one provision way for constructing {@literal Script} objects: via
 * {@link ws.rocket.sqlstore.script.read.ScriptReader}.
 */
