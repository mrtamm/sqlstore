 ...
 * executor.
 *
 * <p>Connection managers focus on opening and closing connections and transactions. Except for the
 * <code>readOnly</code> parameter, a manager typically never configures connection settings.
 *
 * <p>This interface and implementing classes are only used by SqlStore itself, and users of the
...
public interface ConnectionManager {
   ...
   *
   * <p>This method may be called (even multiple times) when a connection has been obtained and
   ...
   *
   * <p>However, note that either commit or roll-back is expected to be called beforehand to store
   * data changes, if necessary, as this method typically just closes the connection (depending on
   * implementation).
   *
   * <p>This method may be called multiple times. When no active connection exists, this method just
