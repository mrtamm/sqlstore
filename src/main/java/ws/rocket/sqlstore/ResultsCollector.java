...
public interface ResultsCollector {
   ...
   * {@link ws.rocket.sqlstore.result.MapResultsCollector}, which supports two values per row (key
   * and its value). Therefore, column index is needed to show the position where the value needs to
   * be stored (e.g. for <code>MapResultsCollector</code>, index 0 stores the value as the key of
   * the map, 1 as the corresponding map value). ResultsCollector container must validate the column
   * index to spot suspicious behaviour. That includes storing a value for same column multiple
   * times.
   *
